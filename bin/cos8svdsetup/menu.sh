...
print_menu() {
	...
	echo -e
	echo -e "  1. Пакет программ и репозиториев для удобства работы с сервером (рекомендуется целиком)."
	echo -e "     Введите 1 чтобы увидеть весь список или установите некоторые из них по отдельности:"
	echo -e " -- 1a \"Webmin\" Панель управления сервером: # webmin_install"
	echo -e " -- 1b \"Bat\" Подсветка синтаксиса кода: # bat_install"
	echo -e " -- 1c \"Snapd\" - это менеджер пакетов и система управления программными приложениями\n    для операционных систем на базе Linux. # snap_install"
	echo -e
    ...
    echo -e
	echo -e "  u. Утилиты."
	echo -e " -- u1 sis - Выполнение поиска текста в теле скриптов *.sh.\n    По умолчанию расширение файлов *.sh, а путь "/". # sis [\"ключевые слова в кавычках\"]"
	echo -e " -- u2 wis - Поиск программы/файла_скрипта локально и в repo,\n    по 6 утилитам. # wis -ключ [имя_программы_скрипта_функции]"
	echo -e " -- u3 ww - Поиск программы/файла_скрипта по 4 командамю # ww [имя_программы_скрипта_функции]"
	echo -e " -- u4 wport - Покажет статистику сетевых портов и соединений,\n    включая информацию о процессах, использующих эти порты. По умолчанию # wport [.] # wport_help"
	echo -e " -- u5 \"dtfex\" - \"качалка файлов\" с файло-обменников\n    Основным достоинством которой является скачивание этих файлов через сеть TOR. # dtfex и # dtfex_help"
	echo -e
	echo -e "  h. Заметки."
...
handle_option() {
	...
	case $choice in
		1)
			...
			;;
		1a)
			echo -e " Вы выбрали Опцию 1a"
			echo -e " -- 1a \"Webmin\" Панель управления сервером: # webmin_install"
			echo -e 
			webmin_install ;
			;;
		1b)
			echo -e " Вы выбрали Опцию 1b"
			echo -e " -- 1b \"Bat\" подсветка синтаксиса кода: # bat_install"
			echo -e 
			bat_install ;
			;;
		1c)
			echo -e " Вы выбрали Опцию 1c"
			echo -e " -- 1c \"Snapd\" - это менеджер пакетов и система управления программными приложениями\n    для операционных систем на базе Linux. # snap_install_help"
			echo -e 
			snap_install_help ;
			snap_install ;
			;;
