...
ttb=$(echo -e "
 ...
 dtfex приведет список URL в полный порядок перед работой, но удалит битые URL 
 без предупреждений, оставив только то, с чем сможет работать.
 ...
 которые поддерживает dtfex, по всей видимости \"их начинка\" идентична.
