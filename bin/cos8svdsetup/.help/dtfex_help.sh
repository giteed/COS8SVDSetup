...
ttb=$(echo -e "
 ...
 символы. Умеет сортировать список URL, а так-же проверяет его на уникальность, по этому вы можете наполнять текстовый файл input.txt в любом виде. dtfex приведет в полный порядок перед работой.
