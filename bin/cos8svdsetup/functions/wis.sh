...
function _help() {
 ...
 | исполняемых файлов, исходных кодов и страниц 
 | руководства (man-страниц) заданной команды 
...
function _provides() {
 ...
 ⎩ сервере репозитариях пакетного менеджера yum/dnf ")  && lang=cr && bpn_p_lang ;
...
function _type() {
	arg_2=("$1")
	 ttb=$(echo -e "   
 ⎧ type - В отличие от which, НЕ осуществляет 
 | сразу поиск в переменой окружения \$PATH
 | type - Показывает значение искомой 
 | команды или алиаса. 
...
function _search() {
	...
	if [[ $arg_2 == "" ]] ; then ttb=$(echo -e "\n ⎧ Укажите вторым аргументом, что требуется найти!\n ⎩ Например: wis -s nginx \n") && lang=cr && bpn_p_lang && return ; fi ;
	 ...
	 ttb=$(echo -e "   
 ⎧ yum search позволяет искать пакеты 
 | по ключевым словам в именах пакетов, 
 | описаниях и дополнительных метаданных.
...
function _all() {
 ...
 | результатов поиска --locate файлов/папок 
 Ё совпадающих с: "$arg_2"
 ...
 ⎩ или: wis --locate" "$arg_2 ") && lang=bash && bpn_p_lang ;	 echo ;
  ...
  echo ;
	 ...
	 ttb=$(echo -e "   
  ⎧ wis -a или --all - Выводит только 25 
  | первых результатов поиска yum search 
  | совпадающих с: "$arg_2" Для вывода
  | всего списка совпадений в yum search:
