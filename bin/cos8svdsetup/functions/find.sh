...
function kill_help() {
   ...
   ttb=$(echo -e " 
    ...
    ⎩ killall и опцию -9 без крайней необходимости!
       ...
       Убедитесь, что программа которую вы хотите убить,
      не находится в фоновом режиме.
       ----------------------------------------------
    ⎧ Если программа была запущена в фоновом режимеб, &, ctrl+z, bg, 
    | то ее можно достать на передний план с помощью команды fg:
    |
