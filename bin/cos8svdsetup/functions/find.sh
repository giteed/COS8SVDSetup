...
sis() {
     ...
     function help_sis() {
       ...
       echo -e " Пример для поиска: sis "получите постоянную скидку" *.md  (ищет текст в *.md файлах) " 
       echo -e "                     sis "получите постоянную скидку" *.md . (ищет текст в *.md файлах в текущей папк) "
