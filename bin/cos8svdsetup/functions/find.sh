...
sis() {
     # Функция справки
     function help_sis() {
       echo -e "\n Выполнение поиска текста в теле скриптов *.sh. По умолчанию расширение файлов *.sh, а путь \"/\"\n Поиском можно искать текст и в обычных текстовых файлах указав тип файла в команде.\n"
       ...
       echo -e "                  sis5 - Команда которая делает тоже самое, но еще выводит 5 строк текста из файлов "
...
sis5() {
     # Функция справки
     function help_sis5() {
       echo -e "\n Выполнение поиска текста в теле скриптов *.sh, с выводом 5 строк. По умолчанию расширение файлов *.sh, а путь \"/\"\n Поиском можно искать текст и в обычных текстовых файлах указав тип файла в команде.\n "
       echo -e " Примеры          sis5 \"получите постоянную скидку\" *.md   (ищет текст в *.md файлах) " 
       echo -e " для поиска:      sis5 \"получите постоянную скидку\" *.md . (ищет текст в *.md файлах в текущей папке) "
       echo -e "                  sis5 \"получите постоянную скидку\"        (ищет текст в *.sh файлах от корня /) "
       echo -e "                  sis5 sis                                 (ищет текст в *.sh файлах от корня /) "
       echo -e "                  sis - Команда которая делает тоже самое, но не выводит текст из файлов "
   
       echo -e "\n Использование: sis5 <паттерн> [<тип_файла>] [<директория>]  "
       echo -e "  <паттерн>     - Строка для поиска (\"заключайте паттерн в кавычки\" если есть пробелы и спец символы)"
       echo -e "  <тип_файла>   - Тип файла (по умолчанию: \"*.sh\" если не указан другой)"
       echo -e "  <директория>  - Путь к директории поиска (по умолчанию: \"/\" если не указан другой)"
