...
sis() {
     # Функция справки
     function help_sis() {
       echo -e "\n Выполнение поиска текста в теле скриптов. По умолчанию расширение файлов *.sh, а путь /\n"
       echo -e " Пример           sis "получите постоянную скидку" *.md   (ищет текст в *.md файлах) " 
       echo -e " для поиска:      sis "получите постоянную скидку" *.md . (ищет текст в *.md файлах в текущей папк) "
       echo -e "                  sis "получите постоянную скидку" (ищет текст в *.sh файлах от корня /) "
       echo -e "\n Использование: sis [<директория>] <паттерн> "
       echo -e "  <директория>  - Путь к директории поиска (по умолчанию: \"/\")"
