...
function msg3() {
	...
	ttb=$(echo -e "	bash /root/vdsetup.2/bin/utility/install/tor/.cash_var_update_now.sh ;\n") && lang_nix && bpn_p_lang ; 
	ttb=$(echo -e "\n Updating now! \n") && lang_nix && bpn_p_lang ;
return ; } ; msg3=(msg3)
...
function update_cash() {
		...
		/root/vdsetup.2/bin/utility/install/tor/.cash_var_update_now.sh ;
...
# Смотрим его статус systemctl status -n0 cash_var.service
