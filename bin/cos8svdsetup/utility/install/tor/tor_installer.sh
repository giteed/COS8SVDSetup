...
. /root/.bashrc
	...
	function wgetrc_config_edit_now() {
		...
		echo "secureprotocol = auto" >> /etc/wgetrc ;
		echo "http_proxy = http://localhost:8118" >> /etc/wgetrc ;
		echo "https_proxy = https://localhost:8118" >> /etc/wgetrc ;
	...
	function privoxy_config_edit_now() {
		...
		# выполняет поиск строки в файле конфигурации privoxy "/etc/privoxy/config", которая начинается с "127.0.0.1:9050 ". Затем с помощью утилиты sed удаляется символ "#" в начале строки (если он присутствует), чтобы активировать эту строку.
		# /127.0.0.1:9050 ./ - это регулярное выражение, которое ищет строку, 
		# содержащую подстроку 127.0.0.1:9050 ., где . - это любой символ;
		# s/^/#/ - это команда sed, которая заменяет начало строки (^) на символ #;
		# -i - это опция sed, которая позволяет изменять файлы в режиме "in place" (то есть в самом файле);
		# /etc/privoxy/config - это путь к обрабатываемому файлу.
		# Таким образом, данная команда заменит начало строки 
		# в файле /etc/privoxy/config, содержащей подстроку 127.0.0.1:9050 .,
		# на символ #, тем самым закомментируя эту строку.
		sed '/127.0.0.1:9050 ./s/^#//' -i.OLD /etc/privoxy/config ; 
		
		
		# закомментировать обратно эту строку:
		# sed - это команда для потокового редактирования текста в Linux.
		# /127.0.0.1:9050 ./ - это регулярное выражение, которое указывает на строки, 
		# содержащие подстроку "127.0.0.1:9050 " с любыми последующими символами.
		# s/^#// - это команда s, которая означает замену, ^# - это регулярное выражение, 
		# которое соответствует символу в начале строки, а пустая строка после // - это замена на пустоту.
		# -i.OLD - это опция для сохранения оригинального файла с расширением .OLD перед внесением изменений.
		# /etc/privoxy/config - это файл, который нужно отредактировать.
		
		# закомментировать обратно эту строку:
		# sed '/127.0.0.1:9050 ./s/^/#/' -i /etc/privoxy/config ;
