...
function core_grubby() {

echo -e " Версия ядра "$(uname -r)" \n Обновление ядра linux не требуется.\n Переходим к установке WireGuard.\n Если ядро уже обновлялось и WireGuard уже был установлен и работал нормально, а теперь перестал, возможно, что загрузка системы снова переключилась на старое ядро. 

 Чтобы переключить загрузку ядра на более свежее выполните следующие действия вручную:
 ...
 Вы получите спискок доступных для загрузки ядер операционной системы, которые находятся в директории /boot/
  Пример вывода: 
 ...
 Каждая строка списка содержит информацию об одном ядре, включая права доступа, владельца и группу, размер, дату изменения и имя файла. 

 Обычно, каждый образ ядра имеет уникальное имя, которое включает в себя номер версии ядра, например /boot/vmlinuz-6.2.10-1.el8.elrepo.x86_64. Это позволяет пользователю выбирать, какое ядро загрузить при запуске операционной системы, если на компьютере установлено несколько ядер.

 2) Введите:  
 # grubby --info [kernel-filename] | grep index

 Это выводит индекс ядра (kernel index) для указанного образа ядра [kernel-filename].
 Пример ввода:
 ...
 Эта команда устанавливает в качестве ядра по умолчанию ядро с индексом 1 в списке доступных ядер на компьютере.
...
function core_ch() {
	echo ; uname -r | grep -qE '^4\.' && echo -e " Версия ядра "$(uname -r)" не поддерживает WireGuard\n Перед установкой WireGuard вам нужно обновить ядро Linux CentOS." && core_up || core_grubby ;
}
...
function manageMenu() {
	...
	0)
		echo -e "\n	The git repository is available at:\n https://github.com/angristan/wireguard-install" ; exit 0
		;;
	esac
}
