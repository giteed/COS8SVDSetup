...
echo -e " Output of 'ls -l /boot/vmlinuz-*':"
...
then
  ...
  then
	echo -e " Kernel version is less than 6, running core_grubby_help"
	core_grubby_help ;
  fi
else
  ...
  if [ "$(uname -r | cut -d'.' -f1)" -lt 6 ]
  then
	echo -e " Kernel version is less than 6 and vmlinuz-6.* is not found, running core_up"
