...
function check_self_connect() {
	...
	function msg_yes() {
		ttb=$(echo -e " Ваш ip \"Напрямую\": $ip_tmp\n | соответствует полученному\n | от \"ifconfig\" ip : $(ifconfig_real_ip)\n |\n | Это значит что анонимайзер или прокси для всей системы\n | выключены и роутинг соединений идет без \n | изменений через сеть вашего провайдера.\n |\n | Если хотите включить TOR для всей системы \n | введите: # toriptables2.py -l\n | Ваш ip \"Напрямую\" проверялся через:") && bpn_p_lang 
	}
