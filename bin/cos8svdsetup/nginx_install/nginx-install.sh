...
source /root/.bashrc
	...
	["$RED" "$(im)""$NC"@"$GRAY"""$(hostname)" ""$NC"] "$NC"<<< "$RED"# "$NC""$GREEN""
		...
		# а так короче:
		# модификатор регулярного выражения i, который означает, что регулярное выражение будет нечувствительным к регистру. Здесь символ "|" используется для объединения двух регулярных выражений. Второе регулярное выражение ^YES$ соответствует строке "YES" без учета регистра. Таким образом, это условие будет соответствовать строкам "y", "Y", "yes", "Yes", "YES".
		#if [[ "$nginxyes" =~ ^yY?$|^YES$ ]]
		#if [[ "$nginxyes" =~ ^y(es)?$|^Y(es)?$ ]] 
		if [[ "$nginxyes" =~ ^[yY][eE]?[sS]?$ ]]
			then 
